...
private:
	...
	public:
		...
		std::vector<S> varIX;
	public:
		Region(T c_r, PotentialVector *pot, S potSize, const std::vector<S>& varIX) : c_r(c_r), pot(pot), potSize(potSize), tmp(NULL), belief(NULL), varIX(varIX) {};
		virtual ~Region() {};
	...
	struct MPNode : public Region {
		MPNode(T c_r, const std::vector<S>& varIX, PotentialVector *pot, S potSize) : Region(c_r, pot, potSize, varIX) {};
...
public:
	...
	const RegionID AddRegion(T c_r, const std::vector<S>& varIX, const PotentialID& p) {
		...
		Graph.push_back(new MPNode(c_r, varIX, &Potentials[p.PotID], Potentials[p.PotID].size));
	...
	T ComputeMu(T* lambdaBase, EdgeID* edge, S* indivVarStates, size_t numVarsOverlap, T epsilon, T* workspaceMem, S* MuIXMem) {
		...
		for (size_t s_p = 0; s_p != s_p_e; ++s_p) {
			...
			T buf = (p_ptr->pot->data == NULL) ? T(0) : p_ptr->pot->data[s_p_real];
	...
	void ReparameterizeRegion(T* lambdaBase, int r, T epsilon, bool additiveUpdate, RRegionWorkspaceID& wspace) {
		...
		for (size_t s_r = 0; s_r != s_r_e; ++s_r) {
			T phi_r_x_r_prime = (r_ptr->pot->data == NULL) ? T(0) : r_ptr->pot->data[s_r];
	...
	T ComputeReparameterizationPotential(T* lambdaBase, const MPNode* const r_ptr, const S s_r) const {
		T potVal = ((r_ptr->pot->data != NULL) ? r_ptr->pot->data[s_r] : T(0));
